...
@patch('nicegui_atlas.commands.info.ComponentAtlas')
def test_info_command_single_component(mock_atlas, info_command, mock_component, capsys):
    """Test info command with a single component."""
    # Set up mock
    mock_atlas.get_component.return_value = mock_component
...
@patch('nicegui_atlas.commands.info.ComponentAtlas')
def test_info_command_with_filter(mock_atlas, info_command, mock_component, capsys):
...
@patch('nicegui_atlas.commands.info.ComponentAtlas')
def test_info_command_with_output_file(mock_atlas, info_command, mock_component, tmp_path):
...
@patch('nicegui_atlas.commands.info.ComponentAtlas')
def test_info_command_no_components_found(mock_atlas, info_command, capsys):
    """Test info command when no components are found."""
    # Set up mock to return None
    mock_atlas.get_component.return_value = None
...
@patch('nicegui_atlas.commands.info.ComponentAtlas')
def test_info_command_multiple_components(mock_atlas, info_command, mock_component, capsys):
